...
- `less` / `vim` / `tmux` / `zellij` / `screen` などフルスクリーンアプリと自然に動く
...
- **ディスク永続化** — 実行中の PTY プロセスの保存は非現実的。予期しないクラッシュ
  （OOM kill・SIGKILL 等）でセッションは失われる想定で使ってほしい（systemd 等で
  自動再起動。tmux/screen を内側で使えば二重の永続化）。計画的な再起動（バイナリ
  更新時）は無停止再起動（`kill -USR2`）で PTY・出力バッファ・接続状態を引き継げるが、
  恒久的な永続化機構ではない
